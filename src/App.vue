...
import { createHoldHotkeyController, toGlobalShortcut } from './domain/hotkeyRecorder';
...
async function registerRuntimeHotkey(hotkey?: string) {
  ...
  if (!hotkey || configuringHotkey.value) return;
...
onMounted(() => {
  ...
  void refreshAll().then(() => registerRuntimeHotkey(config.value?.hotkey));
});
